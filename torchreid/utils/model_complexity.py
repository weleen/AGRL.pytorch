...
def compute_model_complexity(model, input, verbose=False, only_conv_linear=True):
    ...
    Args:
        ...
        input (torch.Tensor or list of torch.Tensor): input
    ...
    if next(model.parameters()).is_cuda:
        if isinstance(input, list):
            input = [inp.cuda() for inp in input]
        else:
            input = [input.cuda()]
    model(*input)  # forward
    ...
    if verbose:
        ...
        print('  Model complexity with input size {}'.format(input[0].size()))
        print('  {}'.format('-'*num_udscore))
