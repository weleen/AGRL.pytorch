...
def visualize_ranked_results(distmat, dataset, save_dir='log/ranked_results', topk=20):
    ...
    for q_idx in range(num_q):
        ...
        if isinstance(qimg_path, list) or isinstance(qimg_path, tuple):
            qimg_index = qimg_path[0].split('/')[-2]
        else:
            qimg_index = osp.basename(qimg_path)
        qdir = osp.join(save_dir, 'id' + qimg_index + '_cam' + str(qcamid))
