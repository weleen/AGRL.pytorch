...
class ResNet50TP(nn.Module):
    def __init__(self, num_classes, loss, block, layers, last_stride=1,
                 bnneck=True, **kwargs):
        self.inplanes = 64
        ...
        self.part = 4
        self.avg_pool = nn.AdaptiveAvgPool2d((self.part, 1))
    ...
    def forward(self, x, adj):
        ...
        # global feature
        v_g = self.avg_pool(x).view(b, s, self.feature_dim, self.part)
        t_a = F.normalize(v_g.norm(p=2, dim=2, keepdim=True), p=1, dim=1)
        f = v_g.mul(t_a).sum(dim=1)
        f = F.adaptive_avg_pool1d(f, 1).view(b, -1)
...
def res50tp(num_classes=100, loss={'xent', 'htri'}, pretrain=True, bnneck=True, last_stride=1, **kwargs):
    model = ResNet50TP(
